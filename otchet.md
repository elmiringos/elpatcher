# Отчёт: ElPatcher - AI Code Agent

## 1. Обзор проекта

**ElPatcher** — это AI-агент для автоматизации жизненного цикла разработки ПО (SDLC) на GitHub. Агент автоматически обрабатывает issues, генерирует код, создаёт pull requests и проводит code review.
...
│  │ (issues/opened)│  │ (pr/opened)    │  │ (@elpatcher fix)│  │
│  └───────┬────────┘  └───────┬────────┘  └───────┬───────┘  │
└──────────┼───────────────────┼───────────────────┼──────────┘
           │                   │                   │
           ▼                   ▼                   ▼
...
### 9.1 RAG с PostgreSQL + pgvector

**Проблема:** Текущая реализация передаёт весь контекст кодовой базы в каждый LLM запрос, что приводит к высоким затратам на inference.

**Решение:** Внедрение полноценного RAG (Retrieval-Augmented Generation):
...
│     ├── Chunking по функциям/классам                        │
│     └── Embedding → pgvector                                │
│                                                             │
│  2. Retrieval при запросе                                   │
│     ├── Query embedding                                     │
│     ├── Similarity search в pgvector                        │
│     └── Top-K релевантных chunks                            │
│                                                             │
│  3. Generation                                              │
...
- OpenAI/Anthropic embeddings или open-source альтернативы
- LangChain Retrievers для интеграции

**Преимущества:**
- Снижение стоимости inference на 60-80%
- Возможность работы с большими репозиториями
- Кэширование embeddings между запросами

### 9.2 Sandbox-контейнеры для изоляции агентов

**Проблема:** Агент имеет доступ к файловой системе сервера и потенциально может получить доступ к чувствительным данным.

**Решение:** Выполнение агента в изолированном sandbox-контейнере:
...
│                                                             │
│  ┌─────────────┐      ┌──────────────────────────────────┐  │
│  │   Server    │      │     Sandbox Container            │  │
│  │             │      │  ┌───────────────────────────┐   │  │
│  │  Webhook ───┼──────┼─▶│  Agent Execution          │   │  │
│  │  Handler    │      │  │  - Cloned repo only       │   │  │
...
**Реализация:**
- Docker-in-Docker или Kubernetes Jobs
- Монтирование только клонированного репозитория
- Ограничение сетевого доступа (только LLM API)
- CPU/Memory limits
- Принудительный timeout

**Преимущества:**
- Изоляция от хост-системы и других процессов
- Защита от malicious code в обрабатываемых репозиториях
- Возможность безопасного выполнения тестов
...
ElPatcher демонстрирует современный подход к созданию AI-агентов для автоматизации разработки:
