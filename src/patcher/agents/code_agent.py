...
IMPORTANT: Only fix what's needed to pass CI. Don't refactor or add new features."""
    ...
    async def _get_review_feedback(self, pr_number: int) -> str | None:
        ...
        try:
            ...
            # Check for issue comments with @elpatcher mention
            issue_comments = list(pr.get_issue_comments())
            for comment in reversed(issue_comments[-10:]):
                if "@elpatcher" in comment.body.lower() or "/elpatcher" in comment.body.lower():
                    return comment.body
    ...
    def _generate_branch_name(self, issue: IssueData) -> str:
        ...
        slug = slug.strip("-")[:40]
        return f"elpatcher/issue-{issue.number}-{slug}"
...
---
*This PR was automatically generated by ElPatcher AI Agent*
